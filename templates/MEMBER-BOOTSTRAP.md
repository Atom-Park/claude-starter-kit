...
목록 정본은 **[docs/conventions/plugins.md](docs/conventions/plugins.md)** 다. 그 표를 열어 **각 플러그인이 무엇을 주는지 먼저 설명한 뒤, 설치 의사를 확인**한다 — 목록만 던지지 않는다.

- 설치는 `/plugin`. 설치·운영 규칙(무엇을 깔지는 본인이 정한다 · MCP 동봉 플러그인은 `claude mcp add` 불필요 · 프로젝트 공유 MCP)은 위 정본 문서가 갖는다.
